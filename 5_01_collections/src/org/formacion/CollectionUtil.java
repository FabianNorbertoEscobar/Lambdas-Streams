...
public class CollectionUtil {
	...
	public void eliminaNombresConEspacios(List<Persona> original) {
		original.removeIf(p -> p.getNombre().indexOf(' ') != -1);
	}
	...
	public void pasarAMayusculas (List<Persona> original) {
		original.replaceAll(p -> new Persona(p.getNombre().toUpperCase(), p.getApellido1().toUpperCase(), p.getApellido2().map(it -> it.toUpperCase())));
	}
